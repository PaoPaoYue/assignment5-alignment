...
def add_group_normalized_rewards_to_rollout(
    ...
    if normalize_by_std:
        group_stds = raw_rewards.view(-1, group_size).std(dim=1).repeat_interleave(group_size)
        normalized_rewards = (raw_rewards - group_means) / (group_stds + advantage_eps)
        return rollout.zip(ray.data.from_items([{"advantage": adv.item(), "group_mean": mean.item(), "group_std": std.item()} for adv, mean, std in zip(normalized_rewards, group_means, group_stds)]))
    else:
        normalized_rewards = raw_rewards - group_means
        return rollout.zip(ray.data.from_items([{"advantage": adv.item(), "group_mean": mean.item()} for adv, mean in zip(normalized_rewards, group_means)]))

def compute_naive_policy_gradient_loss(
    raw_rewards_or_advantages: torch.Tensor,
    policy_log_probs: torch.Tensor,
) -> torch.Tensor:
    print(raw_rewards_or_advantages)
