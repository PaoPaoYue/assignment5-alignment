...
def expert_sample(
    ...
    total = dataset.count()
    sampled = dataset.random_sample(params.ei_sample_batch / total, seed=params.seed + ei_iteration * 1000).limit(params.ei_sample_batch)
    assert sampled.count() == params.ei_sample_batch
...
if __name__ == "__main__":
    ...
    best_ckpt.to_directory(os.path.abspath("./artifacts/checkpoints/ei_ckpt"))
    logger.info(
        f"Train finished with metrics {best_metrics}, saved checkpoint to {os.path.abspath("./artifacts/checkpoints/ei_ckpt")}."
    )
